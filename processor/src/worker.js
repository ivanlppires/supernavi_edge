    ...
    if (job.type === 'P0') {
      ...
      try {
        ...
        try {
          ...
          if (slide) {
            // Names only come from people (filename parser or rename), so they are
            // always confirmed; the cloud only auto-links confirmed names.
            await getPool().query(
...
async function worker() {
  ...
  console.log(`Version: ${PKG_VERSION} — names come from people; SlideRegistered always carries name_confirmed=true`);
