...
// tilegen-svs.js is deprecated — TILEGEN now runs as a processor job
...
export default async function slidesRoutes(fastify) {
  ...
  fastify.get('/slides', async () => {
    ...
    return {
      items: slides.map(s => ({
        ...
        onDemand: false,               // TILEGEN-first: all tiles pre-generated
  ...
  fastify.get('/slides/by-filename/:filename', async (request, reply) => {
    ...
    if (slide.status === 'ingesting') {
      stage = 'processing';
      message = 'Extraindo metadados e gerando thumbnail...';
      progress = 30;
    } else if (slide.status === 'tilegen') {
      stage = 'tilegen';
      message = 'Gerando pirâmide de tiles...';
      progress = 50;
  ...
  // Get tile (pre-generated by TILEGEN)
  ...
  fastify.get('/slides/:slideId/availability', async (request, reply) => {
    ...
    // With TILEGEN-first, all formats have tiles pre-generated when status=ready
    let levelReadyMax = slide.level_ready_max || 0;

    // Scan disk if DB value seems stale
    ...
    const tilesComplete = slide.status === 'ready';
